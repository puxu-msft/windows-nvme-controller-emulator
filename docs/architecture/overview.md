...
| v2.2 | 2025-12-24 | 新增用户态崩溃检测、文件预分配、事件等待模式 |
...
> - ✅ 用户态服务 (`vnvme-server.exe`): v2 模块化架构 (默认)
> - ✅ 命令行工具 (`vnvmectl.exe`): 基础管理命令
> - ⬜ 测试验证: 待在测试环境中验证
>
> 新增功能 (v2.2):
> - ✅ 用户态崩溃检测 (10秒心跳超时)
> - ✅ 文件预分配 (`--preallocate` + SetFileValidData)
> - ✅ 事件等待模式 (低 CPU 使用)
> - ✅ 优雅关机 (WdfIoQueueStop)
...
| [decisions.md](decisions.md) | 架构决策分析和问题修复记录 |
| [../reference/pcie/config-space.md](../reference/pcie/config-space.md) | PCIe 配置空间仿真细节 |
| [../reference/nvme/controller.md](../reference/nvme/controller.md) | NVMe 寄存器和命令定义 |
