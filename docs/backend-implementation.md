...
本文档详细说明 Virtual NVMe StorPort Miniport 驱动各存储后端的实现细节。
    ...
    _In_ PVNVME_BACKEND_CONFIG Config)
{
    ...
    // 初始化文件路径
    RtlInitUnicodeString(&filePath, Config->File.FilePath);
    
    // 复制文件路径
    ctx->FilePath.Length = filePath.Length;
    ctx->FilePath.MaximumLength = filePath.Length + sizeof(WCHAR);
    ...
    RtlCopyUnicodeString(&ctx->FilePath, &filePath);
...
**优点**: 
- 利用现有 Windows VHD API
- 实现简单，不需要在内核解析 VHD 格式
- 支持所有 VHD/VHDX 功能
...
| 使用场景 | 推荐后端 | 原因 |
|----------|----------|------|
| 开发/测试 | Memory | 性能最高，便于调试，无 I/O 错误 |
| 生产环境 (简单) | File | 持久化，配置简单，依赖少 |
| 企业虚拟化 | VHD | 高级功能 (快照、差异盘、Hyper-V 集成) |
| 分布式存储 | Remote | 网络存储 (iSCSI/NVMe-oF，需额外实现) |

### 后端能力对比

| 能力 | Memory | File | VHD | Remote |
|------|--------|------|-----|--------|
| 读/写 | ✓ | ✓ | ✓ | ✓ |
| 持久化 | ✗ | ✓ | ✓ | ✓ |
| 刷新 | ✓ (空操作) | ✓ | ✓ | ✓ |
...
- 直接内存拷贝，无系统调用开销
- 对于大容量，考虑使用 MmAllocateContiguousMemory 分配连续物理内存
- 使用 ERESOURCE 读写锁允许并发读取

### 文件后端

1. **直接 I/O** (`FILE_NO_INTERMEDIATE_BUFFERING`):
   - 减少双重缓冲
   - 要求缓冲区和偏移量对齐到扇区大小

2. **写透模式** (`FILE_WRITE_THROUGH`):
   - 确保数据安全，写入立即刷新
   - 性能开销较高，仅在需要时启用

3. **异步 I/O**:
   - 使用 IoBuildAsynchronousFsdRequest 或 FltReadFile/FltWriteFile
   - 提高并发 I/O 性能

4. **稀疏文件**:
   - 对于大容量虚拟磁盘，使用稀疏文件节省实际磁盘空间
   - TRIM 操作可以释放已分配的区域

### VHD 后端

- 注意 VHD 格式的块对齐，避免跨块读写导致读改写
- 对于动态 VHD，考虑预分配热点块
- 差异盘适合快照场景，但链过长会影响读性能
...
异步 I/O 与 StorPort 的 `SRB_STATUS_PENDING` 配合使用，在 I/O 完成时调用 `StorPortNotification(RequestComplete, ...)` 通知 StorPort。
