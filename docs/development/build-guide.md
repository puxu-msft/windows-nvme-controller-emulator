...
│   ├── prp.c                       # PRP 解析和数据复制
│   ├── admin_cmd.c                 # Admin 命令处理 (内核态)
│   ├── io_cmd.c                    # I/O 命令处理 (内核态)
│   ├── user_forward.c              # 用户态命令转发
│   ├── storage.c                   # 内核态存储后端
│   │
│   │   ══════════════════════════════════════════════════════
│   │   通用/支持
│   │   ══════════════════════════════════════════════════════
│   ├── utils.c                     # 工具函数
│   ├── debug.c / debug.h           # 调试支持
│   ├── trace.h                     # WPP 跟踪宏
│   ├── vnvme.inf                   # INF 文件
│   └── vnvme.vcxproj               # 项目文件
│
├── vnvme-server/                   # ★ 用户态服务 (v2 架构)
│   ├── main_v2.c                   # 服务入口 (v2 模块化架构)
│   ├── vnvme_server.h              # 服务主头文件
│   ├── types.h                     # 类型定义
│   ├── driver_comm.c/.h            # 与内核驱动通信
│   ├── command_engine.c/.h         # NVMe 命令处理引擎
│   ├── admin_commands.c/.h         # Admin 命令处理
│   ├── io_commands.c/.h            # I/O 命令处理
│   ├── backend.h                   # 后端接口定义
│   ├── backend_common.c            # 后端通用代码
│   ├── backend_memory.c            # 内存后端
│   ├── backend_file.c              # 文件后端
│   ├── config.c/.h                 # 配置文件解析
│   ├── logger.c/.h                 # 日志系统
│   ├── vnvme.conf.example          # 配置文件示例
│   └── vnvme-server.vcxproj        # 项目文件
│
├── vnvmectl/                       # 用户模式管理工具
│   ├── main.c                      # CLI 入口 + 所有命令实现
│   └── vnvmectl.vcxproj            # 项目文件
│
├── include/
...
│
├── tests/
│   ├── test-driver.ps1             # 驱动功能测试脚本
│   ├── test_driver_comm.c          # 驱动通信测试
│   └── vnvme-tests.vcxproj         # 测试项目
│
├── docs/                           # 文档
│
├── scripts/
...
Project("{8BC9CEB8-8B4A-11D0-8D11-00A0C91BC942}") = "vnvme", "vnvme\vnvme.vcxproj", "{GUID-1}"
EndProject
Project("{8BC9CEB8-8B4A-11D0-8D11-00A0C91BC942}") = "vnvme-server", "vnvme-server\vnvme-server.vcxproj", "{GUID-2}"
EndProject
Project("{8BC9CEB8-8B4A-11D0-8D11-00A0C91BC942}") = "vnvmectl", "vnvmectl\vnvmectl.vcxproj", "{GUID-3}"
EndProject
Global
...
if ($LASTEXITCODE -eq 0) {
    ...
    Copy-Item "vnvme\x64\$Configuration\vnvme\*" $outputDir -Recurse -Force
    Copy-Item "vnvme-server\x64\$Configuration\vnvme-server.exe" $outputDir -Force
    Copy-Item "vnvmectl\x64\$Configuration\vnvmectl.exe" $outputDir -Force
...
SignTool sign /v /s My /n "VNVME Test Certificate" /t http://timestamp.digicert.com `
    output\Release\vnvme.sys

# 签名 cat 文件
SignTool sign /v /s My /n "VNVME Test Certificate" /t http://timestamp.digicert.com `
    output\Release\vnvme.cat
```
...
$vnvmeInf = Join-Path $PSScriptRoot "..\$OutputDir\vnvme.inf"

if ($Action -eq "install") {
    Write-Host "Installing VNVME driver..."
    ...
    # 安装驱动
    Write-Host "Installing vnvme driver..."
    pnputil /add-driver $vnvmeInf /install
    
    Write-Host "Installation complete!" -ForegroundColor Green
    
} else {
    Write-Host "Uninstalling VNVME driver..."
    
    # 卸载驱动
    pnputil /delete-driver $vnvmeInf /uninstall /force
