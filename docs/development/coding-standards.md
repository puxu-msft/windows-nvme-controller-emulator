...
| 模块 | 函数命名 | 局部变量 | 结构体前缀 | 语言标准 |
|------|----------|----------|------------|----------|
| vnvme (内核驱动) | PascalCase (`VnvmeXxx`) | camelCase | `VNVME_` | C17 |
| vnvme-server (用户态服务) | PascalCase (`AdminCmd*`, `IoCmd*`) | camelCase | `*_CONTEXT`, `*_STATE` | C++23 |
| vnvmectl (管理工具) | PascalCase (`Command*`) | camelCase | `VNVME_` | C17 |
| include (公共头文件) | PascalCase | - | `VNVME_` | 兼容 C17/C++23 |

### 9.3 内核驱动 (vnvme/) 风格

- **函数前缀**: `Vnvme` (如 `VnvmeCreateController`)
- **回调前缀**: `VnvmeEvt` (如 `VnvmeEvtDriverUnload`)
- **模块前缀**: 按功能分类 (如 `Queue*`, `Shm*`, `Admin*`)
- **全局变量**: `g_` 前缀 (如 `g_DriverObject`)
- **宏常量**: `VNVME_` 前缀 + UPPER_SNAKE_CASE

### 9.4 用户态服务 (vnvme-server/) 风格

- **模块函数**: 模块名前缀 (如 `AdminCmdInit`, `IoCmdHandleRead`)
- **内部静态函数**: 不需要前缀，但应使用描述性名称
- **日志宏**: `LOG_INFO`, `LOG_WARN`, `LOG_ERROR`
- **结构体命名**: 描述性名称 + 用途后缀 (如 `CMD_ENGINE_CONTEXT`)

### 9.5 管理工具 (vnvmectl/) 风格

- **命令函数**: `Command` 前缀 (如 `CommandCreate`, `CommandStatus`)
- **辅助函数**: 描述性名称 (如 `PrintUsage`, `ParseArgs`)
- **全局常量**: 模块前缀 + 描述 (如 `VNVMECTL_VERSION`)
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2025-12-24 | 1.2 | 添加模块特定代码风格章节 (分隔符注释、模块风格矩阵等) |
