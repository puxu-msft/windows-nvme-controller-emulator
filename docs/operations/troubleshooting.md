...
> **注意**：v2 架构使用单一内核驱动 `vnvme.sys`，合并了原来的 bus 和 emu 功能。
...
# 检查签名
signtool verify /pa /v vnvme.sys
```
...
2. **重新签名驱动**
   ```powershell
   # 使用测试证书签名
   signtool sign /v /s My /n "VNVME Test Certificate" /t http://timestamp.digicert.com vnvme.sys
   ```
...
3. **检查 Admin 队列初始化**
   ```
   # 在 WinDbg 中设置断点
   bp vnvme!VnvmeInitializeAdminQueues
   bp vnvme!VnvmeProcessCreateIoCq
   bp vnvme!VnvmeProcessCreateIoSq
   ```
...
使用 WinDbg：
```
bp vnvme!VnvmeProcessRead
bp vnvme!VnvmeProcessWrite
```
...
# 使用驱动验证器池跟踪
verifier /flags 0x100 /driver vnvme.sys
```
