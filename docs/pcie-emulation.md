...
本文档详细说明虚拟 PCIe 总线驱动 (vnvme_bus.sys) 的设计和实现。
...
└───────────────────────────┬─────────────────────────────────────┘
                            ...
                            ▼
┌─────────────────────────────────────────────────────────────────┐
│              虚拟 NVMe 控制器 PDO (由 bus 创建)                   │
│              Hardware ID: PCI\VEN_1B36&DEV_0010...               │
└───────────────────────────┬─────────────────────────────────────┘
                            │ stornvme.sys 作为 FDO 加载
                            ▼
┌─────────────────────────────────────────────────────────────────┐
│                    stornvme.sys (FDO)                           │
│                    Windows 原生 NVMe 驱动                        │
...
[VnvmeBus_Device.NT]
CopyFiles = VnvmeBus_Device.CopyFiles

[VnvmeBus_Device.CopyFiles]
vnvme_bus.sys

[VnvmeBus_Device.NT.Services]
AddService = vnvme_bus, 0x00000002, VnvmeBus_Service

[VnvmeBus_Service]
DisplayName    = %VnvmeBus.ServiceDesc%
ServiceType    = 1               ; SERVICE_KERNEL_DRIVER
StartType      = 3               ; SERVICE_DEMAND_START
ErrorControl   = 1               ; SERVICE_ERROR_NORMAL
ServiceBinary  = %13%\vnvme_bus.sys

[Strings]
ManufacturerName       = "Virtual NVMe Project"
VnvmeBus.DeviceDesc    = "Virtual NVMe PCIe Bus"
VnvmeBus.ServiceDesc   = "Virtual NVMe PCIe Bus Driver"
...
```powershell
# 创建根枚举设备
devcon install vnvme_bus.inf ROOT\VNVMEBUS
