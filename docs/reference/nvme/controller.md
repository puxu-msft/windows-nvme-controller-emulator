...
本文档详细说明 NVMe 控制器仿真的设计和实现。

> **v2 注意**: 以下内容描述 NVMe 控制器概念。实际实现位于 `vnvme.sys` 的 PDO 层代码中。

## 概述

vnvme.sys 的 PDO 层是本项目的核心，负责完整仿真 NVMe 控制器的行为，使 Windows 原生 NVMe 驱动 (stornvme.sys) 能够正常工作。
