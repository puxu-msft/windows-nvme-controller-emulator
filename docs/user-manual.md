...
```powershell
# 创建控制器
vnvmectl.exe create --id <controller_id>

# 删除控制器
vnvmectl.exe delete --id <controller_id>
...
```powershell
# 创建命名空间
vnvmectl.exe ns create --controller <id> --nsid <nsid> --size <size>

# 删除命名空间
vnvmectl.exe ns delete --controller <id> --nsid <nsid>

# 列出命名空间
vnvmectl.exe ns list --controller <id>
```
...
2. 使用 WinDbg 分析：
   ...
   .load vnvme.sys
   ```
3. 参考 [debugging-guide.md](debugging-guide.md) 进行调试
